/* ============================================================
 * TANJA OS BIN COMMAND - vi
 * ------------------------------------------------------------
 * A full vi for TanjaOS.  Compiled by the Makefile with plain
 * gcc -m32 (freestanding) and embedded in the kernel image
 * (via home.tar) as a /bin command at boot.
 *
 * NORMAL MODE
 *   motions     h l j k arrows  0 ^ $ w W b B e E  ( ) { }
 *               gg G (5G = goto line 5)  %  f/F/t/T + char  ; ,
 *               'x `x '' (marks)  CR = first non-blank of next line
 *   scrolling   ^F ^B ^D ^U  z<CR> zz z- zt zb  ^G (file info)
 *   operators   d c y < > with any motion above, dd cc yy << >>
 *               counts before and after: 3dw == d3w
 *   changes     x X s S r R ~ J D C Y p P  . (repeat)  & (repeat :s)
 *               u (multi-level undo)  ^R (redo)
 *   registers   "x<op> / "xp  use register x (a-z 1-9, default ");
 *               line deletes auto-number registers 1..9
 *   marks       mx   'x jump to mark line   `x jump to exact spot
 *               '' `` last jump position
 *   search      /pat ?pat n N  (mini-regex: ^ $ . * [...] \(..\))
 *               counts: 3/pat = 3rd match
 *   ZZ = :x
 *
 * INSERT MODE   typing, Enter (autoindent with :set ai), Backspace,
 *               Tab, arrows, ^W delete word back, Esc ends change.
 * REPLACE MODE  (R) typing overwrites, Esc ends.
 *
 * EX COMMANDS
 *   :w [file] :q :q! :wq :x :e[!] [file] :r [file]
 *   :[range]        goto line
 *   :[range]d  :[range]y  :[range]t<addr>  :[range]m<addr>
 *   :[range]s/pat/rep/[g]    :s (repeat last)   :set nu ai ic ...
 *   ranges: . $ % n,m +n -n 'x     e.g. :1,$s/old/new/g
 *
 * UTF-8 aware gap buffer with tab stops and 80-column wrapping;
 * multi-level undo keeps full snapshots; registers, marks, and a
 * small backtracking regex engine for search and substitute.
 * ============================================================ */
