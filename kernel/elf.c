/* ============================================================
 * TANJA OS - IN-KERNEL ELF32 DRIVER
 * ------------------------------------------------------------
 * Loads real ELF32 i386 relocatable objects (.o files, exactly
 * what `gcc -m32 -ffreestanding -c` produces) and runs them:
 *
 *   1. every SHF_ALLOC section is copied (or zeroed, for .bss)
 *      into the program region at 0x40000000,
 *   2. every symbol defined in the object gets its final address
 *      (section base + st_value); every UNDEFINED symbol is
 *      resolved by name against the kernel export table below
 *      (the include/tanja.h API),
 *   3. R_386_32 / R_386_PC32 relocations are applied,
 *   4. the `main` symbol is called with the command argument string.
 *
 * This is the same machinery a real OS uses for loadable drivers:
 * the "busybox" command set in /Programs and any user program
 * compiled with gcc -m32 (freestanding) are loaded through here.
 * ============================================================ */
