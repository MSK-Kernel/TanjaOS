/* ============================================================
 * TanjaOS entry wrapper for tcc
 * ------------------------------------------------------------
 * TanjaOS programs are entered as `void main(char *args)` (one
 * argument string, like the kernel's own command lines), while
 * tcc is an ordinary program expecting `int main(int, char**)`.
 * tcc's real main is renamed to tcc_main_real at compile time
 * (-Dmain=tcc_main_real); this file owns the name `main`, splits
 * the argument string into argv, and maps exit() onto a non-local
 * jump back here so tcc can terminate from any depth, the same
 * way a real process exit would.
 *
 * TanjaOS also has no linker or C runtime: tcc can only compile
 * to an object file that the kernel ELF loader then runs via
 * `exec`. So when the user asks tcc for a linked executable
 * (no -c/-E/-S/-run, e.g. `tcc -o test test.c`), we transparently
 * downgrade to compile-only, make the output name end in .o, and
 * print how to run it - instead of dying with 'crt1.o not found'.
 * ============================================================ */
